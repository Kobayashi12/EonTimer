...
namespace EonTimer::Timer::Factory {
    std::chrono::milliseconds SecondTimer::createStage1(const u32 targetSecond, const i32 calibration) const {
        return Util::toMinimumLength(std::chrono::milliseconds(targetSecond * 1000 + calibration + 200));
    }

    std::vector<std::chrono::milliseconds> SecondTimer::createStages(const u32 targetSecond,
                                                                     const i32 calibration) const {
        return std::vector<std::chrono::milliseconds>{createStage1(targetSecond, calibration)};
    }

    i32 SecondTimer::calibrate(const u32 targetSecond, const u32 secondHit) const {
